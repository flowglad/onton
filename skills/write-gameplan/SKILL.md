...
[Functional Change Ownership](#functional-change-ownership) makes the *behavior* partition correct — every observable change has exactly one owning patch (total and disjoint). The same discipline must hold for the *file* partition, and each patch must actually do something. Two recurring defects come from skipping this: a patch whose real change spills into files it never listed, and a patch whose planned change was already true (a no-op). Both are detectable at authoring time against the code you already grounded.
...
**Each patch must be non-vacuous.** A patch whose postcondition already holds in the grounded pre-state is a no-op — its spec is satisfied *vacuously*, for a trivial reason rather than the intended one (the textbook vacuity case: "every request is followed by a grant" passes in a system that never makes a request). Before finalizing a patch, check its planned change against the real current surface: if the field already exists, the route is already registered, the type already has the variant, the function already accepts that shape — the patch contributes nothing and must be dropped or rescoped to the work actually missing. The test is mechanical: remove the patch and ask whether its postcondition still holds against the grounded code; if it does, the patch is empty.
