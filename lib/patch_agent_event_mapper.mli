...
(** Total mapping from patch-agent stdio RPC events into onton's canonical
    ...
    variant names. ["stop"] maps to [Types.Stop_reason.End_turn]. Patch-agent's
    ["error"] is also treated as [Types.Stop_reason.End_turn] silently because
    it is an expected value with no direct onton counterpart. Fully unknown
