...
(** [complete ~buffer ~patch_ids] returns completions for the current text input
    buffer. Handles:
...
(** Replace buffer with the first completion's full text, or return buffer
    unchanged if completions is empty. *)
